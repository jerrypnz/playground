...
void test_parse_once() {
    ...
    rc = parser_parse_request(&parser, test_request, req_size, &consumed_size);
...
void test_parse_once_with_extra_data() {
    ...
    rc = parser_parse_request(&parser, test_request_2, req_size, &consumed_size);
    printf("Request size: %d, consumed size: %d, return status: %d\n", req_size, consumed_size, rc);
    dump_parser();
    assert(rc == STATUS_COMPLETE);
...
void test_parse_multiple_times() {
    ...
    // First time
    rc = parser_parse_request(&parser, data, part1_size, &consumed_size);
    ...
    // Second time
    rc = parser_parse_request(&parser, data, part2_size, &consumed_size);
    ...
    // Third time
    rc = parser_parse_request(&parser, data, part3_size, &consumed_size);
