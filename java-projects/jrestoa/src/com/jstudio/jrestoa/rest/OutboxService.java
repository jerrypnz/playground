...
@URLMapping("/mail/outbox/{mail_id}")
public class OutboxService extends AbstractRESTResource
{
			...
			HttpServletResponse response) throws IOException, ServletException
	{
		...
		{
			List<SentMail> sentMails = user.checkSentMail();
			out.println(XMLUtil.xmlHeader);
			String result = XMLUtil.listToXML(sentMails, "outboxMails");
			log.debug(result);
			out.println(result);
		}
		else
		{
			...
			SentMail mail = SentMail.findById(mailId);
